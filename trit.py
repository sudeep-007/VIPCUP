...
@trit.autotune(configs = [
    trit.Config({'BLOCK_SIZE': 128}, num_warps = 4, pre_hook = clone_inplace_updated_params),
    trit.Config({'BLOCK_SIZE': 1024}, num_warps = 8, pre_hook = clone_inplace_updated_params),
], key = ['n_elements'])
@trit.jit
...
def update_fn(
    ...
    grid = lambda meta: (trit.cdiv(n_elements, meta['BLOCK_SIZE']),)    
